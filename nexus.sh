...
sudo apt update && sudo apt upgrade && sudo apt install wget build-essential pkg-config libssl-dev unzip -y && wget https://github.com/protocolbuffers/protobuf/releases/download/v29.1/protoc-29.1-linux-x86_64.zip && unzip protoc-29.1-linux-x86_64.zip -d $HOME/.local && export PATH="${HOME}/.local/bin":${PATH}
